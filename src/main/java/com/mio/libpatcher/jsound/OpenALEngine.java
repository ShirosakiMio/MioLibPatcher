...
final class OpenALEngine {
    ...
    private void run() {
        ...
        if (!ok) {
            ...
            LogUtil.error("[JSound] OpenAL device could not be opened, javax.sound.sampled stays unavailable");
            return;
        }
        ready = true;
        readyLatch.countDown();
        LogUtil.info("[JSound] OpenAL device ready, javax.sound.sampled is now routed to OpenAL");
        while (!shutdown) {
            drainTasks();
            tickVoices();
    ...
    private boolean setupContext() {
        try {
            ...
            if (dev == 0L) {
                LogUtil.error("[JSound] alcOpenDevice failed");
                return false;
            }
            ...
            if (!alcCaps.OpenALC10) {
                ...
                LogUtil.error("[JSound] OpenAL implementation does not support ALC 1.0");
                return false;
            }
            long ctx = ALC10.alcCreateContext(dev, (IntBuffer) null);
            if (ctx == 0L) {
                ALC10.alcCloseDevice(dev);
                LogUtil.error("[JSound] alcCreateContext failed");
                return false;
            }
            if (!ALC10.alcMakeContextCurrent(ctx)) {
                ALC10.alcDestroyContext(ctx);
                ALC10.alcCloseDevice(dev);
                LogUtil.error("[JSound] alcMakeContextCurrent failed");
                return false;
            }
            ...
            LogUtil.error("[JSound] OpenAL context setup failed", t);
            return false;
        }
    }
    ...
    private void destroyContext() {
        try {
            ...
            LogUtil.error("[JSound] Error while closing the OpenAL device", t);
        }
    }
    ...
    private void drainTasks() {
        ...
        while ((task = tasks.poll()) != null) {
            try {
                ...
                LogUtil.error("[JSound] Worker task failed", t);
            }
        }
    }
    ...
    private void tickVoices() {
        for (Voice voice : voices) {
            try {
                ...
                LogUtil.error("[JSound] Voice tick failed", t);
            }
        }
    }
