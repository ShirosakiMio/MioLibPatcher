...
| `oshi.hardware.CentralProcessor$ProcessorIdentifier` / `oshi.software.os.linux.proc.CentralProcessor` (oshi 1.x) / `oshi.hardware.platform.linux.LinuxCentralProcessor` (oshi 6.x) | Reads CPU name from system property `cpu.name`; core count from JVM `-XX:ActiveProcessorCount`; skips device topology probing |
